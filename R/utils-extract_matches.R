...
extract_matches <- function(tree_data, species_data, n) {
  ...
  unique_commons$common_name <- cpp_trimws(unique_commons$common_name, "both") # Save for end?
  ...
  unique_botanicals$botanical_name <- cpp_trimws(unique_botanicals$botanical_name, "both") # Save for end?

  species$common_name_m <- tolower(species$common_name)
  species$common_name_m <- gsub('[[:punct:]]+', '', species$common_name_m)
  species$common_name_m <- cpp_trimws(species$common_name_m, "both") # Save for end?

  species$botanical_name_m <- tolower(species$scientific_name)
  species$botanical_name_m <- gsub('[[:punct:]]+', '', species$botanical_name_m)
  species$botanical_name_m <- cpp_trimws(species$botanical_name_m, "both") # Save for end?
  ...
  trees$common_name <- cpp_trimws(trees$common_name, "both") # Save for end?

  trees$botanical_name <- tolower(trees$botanical_name)
  trees$botanical_name <- gsub('[[:punct:]]+', '', trees$botanical_name)
  trees$botanical_name <- cpp_trimws(trees$botanical_name, "both") # Save for end?

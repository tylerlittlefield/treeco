...
The first thing it checks is whether or not `data` is an object. If it is, it's converted to a `data.table` and if it's not, it's assumed to be a CSV and imported with `fread`:
...
Next, the datasets are saved as `data.table`'s and the money dataset is filtered by region, `melt`ed to convert from wide to long and selects the variables we need:
...
Next, the names of the trees dataset are set to keep everything consistent from this point going forward:
...
Next, if the users assigns `unit = "cm"`, the DBH values will not be converted to centimeters, if no argument is given `unit` defaults to inches and converts DBH values to centimeters:
